...
It may be useful to change the color of the `HelpText` outside of what the `severity` prop offers. Use the `color` prop and the `TextColor` object to change the color of the `HelpText`. Defaults to `COLORS.TEXT_DEFAULT`.
